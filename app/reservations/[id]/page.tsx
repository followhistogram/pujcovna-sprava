...
export default async function ReservationEditPage({ params }: { params: { id: string } }) {
  ...
  try {
    if (!isNew) {
      ...
      const { data: emailData } = await supabase.from("email_logs").select("*").eq("reservation_id", params.id)
    ...
    return (
      <main className="container mx-auto py-6 px-4">
        <div className="text-center py-12">
          <p className="text-muted-foreground">Chyba při načítání rezervace. Zkuste obnovit stránku.</p>
