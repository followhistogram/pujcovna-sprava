// lib/data.ts - Opravená verze bez inventory tabulky
...
// Stávající funkce fetchReservationById - UPRAVENÁ pro lepší handling položek
export async function fetchReservationById(id: string) {
  ...
  try {
    ...
    // Ujistíme se, že items mají správné typy a hodnoty
    if (data?.items) {
...
export async function getTableStructure() {
  try {
    ...
    const possibleTables = [
      'inventory', 'cameras', 'films', 'accessories', 
      'products', 'items', 'equipment', 'gear'
    ]
